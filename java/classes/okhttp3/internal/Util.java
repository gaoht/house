...
public final class Util
{
  ...
  static
  {
    ...
    UTF_8_BOM = f.decodeHex("efbbbf");
    UTF_16_BE_BOM = f.decodeHex("feff");
    UTF_16_LE_BOM = f.decodeHex("fffe");
    UTF_32_BE_BOM = f.decodeHex("0000ffff");
    UTF_32_LE_BOM = f.decodeHex("ffff0000");
  ...
  public static Charset bomAwareCharset(e parame, Charset paramCharset)
    throws IOException
  {
    if (parame.rangeEquals(0L, UTF_8_BOM))
    {
      parame.skip(UTF_8_BOM.size());
      paramCharset = UTF_8;
    }
    do
    {
      return paramCharset;
      if (parame.rangeEquals(0L, UTF_16_BE_BOM))
      {
        parame.skip(UTF_16_BE_BOM.size());
        return UTF_16_BE;
      }
      if (parame.rangeEquals(0L, UTF_16_LE_BOM))
      {
        parame.skip(UTF_16_LE_BOM.size());
        return UTF_16_LE;
      }
      if (parame.rangeEquals(0L, UTF_32_BE_BOM))
      {
        parame.skip(UTF_32_BE_BOM.size());
        return UTF_32_BE;
      }
    } while (!parame.rangeEquals(0L, UTF_32_LE_BOM));
    parame.skip(UTF_32_LE_BOM.size());
    return UTF_32_LE;
  }
  ...
  public static boolean discard(t paramt, int paramInt, TimeUnit paramTimeUnit)
  {
    try
    {
      boolean bool = skipAll(paramt, paramInt, paramTimeUnit);
      return bool;
    }
  ...
  private static String inet6AddressToAscii(byte[] paramArrayOfByte)
  {
    ...
    while (i < paramArrayOfByte.length) {
      ...
      else
      {
        ...
        localc.writeHexadecimalUnsignedLong((paramArrayOfByte[i] & 0xFF) << 8 | paramArrayOfByte[(i + 1)] & 0xFF);
        i += 2;
      }
    }
    return localc.readUtf8();
  }
    ...
    throws IOException
  {
    ...
    for (;;)
    {
      ...
      {
        paramTimeUnit = new c();
        while (paramt.read(paramTimeUnit, 8192L) != -1L) {
          paramTimeUnit.clear();
        }
      }
      catch (InterruptedIOException paramTimeUnit)
      {
        ...
        for (;;)
        {
          ...
          {
            return true;
            paramt.timeout().deadlineNanoTime(l1 + l2);
          }
          paramt.timeout().deadlineNanoTime(l1 + l2);
        }
      }
...
/* Location:              /Users/gaoht/Downloads/zirom/classes-dex2jar.jar!/okhttp3/internal/Util.class
