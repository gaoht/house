  ...
  extends ScrimInsetsFrameLayout
{
  ...
  public NavigationView(Context paramContext, AttributeSet paramAttributeSet, int paramInt)
  {
    ...
    at localat = at.obtainStyledAttributes(paramContext, paramAttributeSet, R.styleable.NavigationView, paramInt, R.style.Widget_Design_NavigationView);
    ViewCompat.setBackground(this, localat.getDrawable(R.styleable.NavigationView_android_background));
    if (localat.hasValue(R.styleable.NavigationView_elevation)) {
      ViewCompat.setElevation(this, localat.getDimensionPixelSize(R.styleable.NavigationView_elevation, 0));
    }
    ViewCompat.setFitsSystemWindows(this, localat.getBoolean(R.styleable.NavigationView_android_fitsSystemWindows, false));
    this.mMaxWidth = localat.getDimensionPixelSize(R.styleable.NavigationView_android_maxWidth, 0);
    ColorStateList localColorStateList;
    if (localat.hasValue(R.styleable.NavigationView_itemIconTint))
    {
      localColorStateList = localat.getColorStateList(R.styleable.NavigationView_itemIconTint);
      if (!localat.hasValue(R.styleable.NavigationView_itemTextAppearance)) {
        break label376;
      }
      paramInt = localat.getResourceId(R.styleable.NavigationView_itemTextAppearance, 0);
    }
    for (int i = 1;; i = 0)
    {
      paramAttributeSet = null;
      if (localat.hasValue(R.styleable.NavigationView_itemTextColor)) {
        paramAttributeSet = localat.getColorStateList(R.styleable.NavigationView_itemTextColor);
      }
      ...
      paramAttributeSet = localat.getDrawable(R.styleable.NavigationView_itemBackground);
      this.mMenu.setCallback(new h.a()
      {
      ...
      if (localat.hasValue(R.styleable.NavigationView_menu)) {
        inflateMenu(localat.getResourceId(R.styleable.NavigationView_menu, 0));
      }
      if (localat.hasValue(R.styleable.NavigationView_headerLayout)) {
        inflateHeaderView(localat.getResourceId(R.styleable.NavigationView_headerLayout, 0));
      }
      localat.recycle();
      return;
  ...
  private ColorStateList createDefaultColorStateList(int paramInt)
  {
    ...
    {
      return null;
      localColorStateList = b.getColorStateList(getContext(), ((TypedValue)localObject).resourceId);
  ...
  public void setCheckedItem(int paramInt)
  {
    ...
    if (localMenuItem != null) {
      this.mPresenter.setCheckedItem((j)localMenuItem);
    }
  }
...
/* Location:              /Users/gaoht/Downloads/zirom/classes-dex2jar.jar!/android/support/design/widget/NavigationView.class
